...
%% Calculate variance due to learning using bootstrapping over trials
    ...
    for filenum = 1:length(filenames)%[1:4 6:11]
    ...
    for arraynum = 1:length(arrays)
        ...
        for sessionnum = 1:length(sessions)
            ...
            for margnum = 1:length(session_table.marg_names)
                ...
                ad_val = session_table.marg_var_pct(margnum);
                bl_val = bl_table.marg_var_pct(margnum);
                learning_shuffle_val =(learning_shuffle_table.marg_var_pct(:,margnum));
...
%                 end
                ...
                plot(...
                    ...
                    '-','color',point_color)
%                 scatter(...
%                     repmat(arraynum-0.1,num_control_shuffles,1),...
