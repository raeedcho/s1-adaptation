...
%       (third order tensor, shape = time_points x learning_dims x trials)
    ...
    if plot_comp_dpcs && isfield(td,sprintf('%s_dpca_%s',spikes_in_td{arraynum},comp_to_plot))
        ...
        title(sprintf('%s %s projection into %s dims',td(1).monkey,strrep(spikes_in_td{arraynum},'_spikes',''),comp_to_plot))
        set(gca,'box','off','tickdir','out')
    end
end